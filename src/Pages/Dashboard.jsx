import React, { useState, useEffect } from "react";
import axios from "axios";
import { DeleteCheckbox } from "../Components/Checkbox";
import { useRecoilState, useRecoilValue } from "recoil";
import { deletedAtom } from "../store/atoms/deletedAtom";

export const Dashboard = () => {
  const [form, setForm] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [deleted, setDeleted] = useRecoilState(deletedAtom);


  useEffect(() => {
    axios.get("https://donor-backend.onrender.com/bulk")
      .then(res => {
        setForm(res.data.form);
        setLoading(false);
      })
      .catch(err => {
        setError(err.message);
        setLoading(false);

      });
  }, [deleted]);

  const handleDelete = (id) => {
    setForm((prevForm) => prevForm.filter(ent => ent._id !== id));
  };

  if (loading) return (
    <div className="p-10 flex justify-center">
      <svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
          <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
      </svg>
      <span className="sr-only">Loading...</span>
    </div>
  );

  if (error) return (
    <div className="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-10" role="alert">
        <p className="font-bold">Be Warned</p>
        <p>Something not ideal might be happening.</p>
    </div>
  );

  return (
    <>
      <div className="flex justify-center font-bold mt-6 text-2xl">
        Donations
      </div>
      <div className="p-4 justify-between grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {form.map(ent => (
          <Users name={ent.name} email={ent.email} number={ent.number} food={ent.food} location={ent.location} quantity={ent.quantity}/>
        ))}
      </div>
    </>
  );
};

const Users = ({ name, email, number, food, location, quantity}) => {
  return (
    <div className="border rounded-lg p-4 bg-white dark:bg-gray-800">
      <div className="inline-flex-start">
        <DeleteCheckbox id={number} onDelete={() => {onDelete}} />
      </div>
      <dl className="grid grid-cols-2 gap-4 text-gray-900 dark:text-white">
        <div className="flex flex-col items-center justify-center">
          <dt className="mb-2 text-xl font-extrabold">{name}</dt>
          <dd className="text-gray-500 dark:text-gray-400">Name</dd>
        </div>
        <div className="flex flex-col items-center justify-center">
          <dt className="mb-2 text-xl font-extrabold">{email}</dt>
          <dd className="text-gray-500 dark:text-gray-400">Email</dd>
        </div>
        <div className="flex flex-col items-center justify-center">
          <dt className="mb-2 text-xl font-extrabold">{number}</dt>
          <dd className="text-gray-500 dark:text-gray-400">Number</dd>
        </div>
        <div className="flex flex-col items-center justify-center">
          <dt className="mb-2 text-xl font-extrabold">{food}</dt>
          <dd className="text-gray-500 dark:text-gray-400">Food Type</dd>
        </div>
        <div className="flex flex-col items-center justify-center">
          <dt className="mb-2 text-xl font-extrabold">{location}</dt>
          <dd className="text-gray-500 dark:text-gray-400">Location</dd>
        </div>
        <div className="flex flex-col items-center justify-center">
          <dt className="mb-2 text-xl font-extrabold">{quantity}</dt>
          <dd className="text-gray-500 dark:text-gray-400">Quantity (in Kgs)</dd>
        </div>
      </dl>
    </div>
  );
};
